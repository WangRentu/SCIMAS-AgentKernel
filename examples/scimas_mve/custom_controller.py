...
class CustomController(ControllerImpl):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._verbose_agent_logs = os.getenv(
            "SCIMAS_VERBOSE_AGENT_LOGS",
            "1" if self._log_mode == "verbose" else "0",
        ).lower() in {"1", "true", "yes"}
    ...
    async def finalize_episode(self, episode_index: int = 0) -> Dict[str, Any]:
        ...
        chain_metrics_path = os.path.join(out_dir, "research_chain_metrics.jsonl")
        ...
        for rec in records:
            ...
            if not isinstance(counts, dict):
                continue
            for action_name in list(action_totals.keys()):
                action_totals[action_name] += int(counts.get(action_name, 0) or 0)
        ...
        chain_record = {
            "ts": datetime.utcnow().isoformat() + "Z",
            "episode_index": episode_index,
            "task_name": (taskboard_metrics or {}).get("task_name"),
            "taskboard_claim_events": claim_events,
            "taskboard_complete_events": complete_events,
            ...
            "action_review": int(action_totals.get("review", 0) or 0),
            "action_replicate": int(action_totals.get("replicate", 0) or 0),
            "action_claim_task": int(action_totals.get("claim_task", 0) or 0),
            "action_share_evidence": int(action_totals.get("share_evidence", 0) or 0),
            "action_share_observation": int(action_totals.get("share_observation", 0) or 0),
            "research_steps_total": int(chain_research_steps),
            "research_steps_per_claim": (float(chain_research_steps) / float(claim_events)) if claim_events else 0.0,
            "collaboration_ratio": collaboration_ratio,
            "team_fitness": team_fitness,
            "active_worker_count": int((taskboard_metrics or {}).get("active_worker_count", 0) or 0),
        }
        team_record["research_steps_total"] = int(chain_research_steps)
        team_record["taskboard_claim_events"] = claim_events
        team_record["taskboard_complete_events"] = complete_events
        ...
        try:
            ...
            with open(chain_metrics_path, "a", encoding="utf-8") as f:
